...
public class CQELSMultiple {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		int noQ=Integer.parseInt(args[7]);
		...
		String outFN=OUTPUT+"/"+q+"/"+q+"-"+noQ+"-"+noU+"-"+strLen+"-0.out";
        ...
        for(int i=0;i<noQ;i++){
	      	ContinuousSelect selQuery=context.registerSelect(query.replace("{CONSTANT}", ""+i));
	      	selQuery.register(new MList(out,context));
	    }
        
		long start=System.currentTimeMillis();
		for(Thread t:players) t.start();
		 while(isAlive(players)&&((System.currentTimeMillis()-start)<3600000)){
			 Thread.sleep(100);
		 }
		 long elapsed=(System.currentTimeMillis()-start);
		 out.println("Number of results "+MList.count +" in "+elapsed +" milliseconds");
		 long triples=0;
